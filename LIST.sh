#!/bin/bash

# ${SRCDIR}/a01.ListPrep.sh
# ${SRCDIR}/a01.ListPrep_HandPick.sh
# ${SRCDIR}/a02.MasterMap.sh
# ${SRCDIR}/a03.CityMap.sh
# ${SRCDIR}/a04.Histogram.sh
${SRCDIR}/a05.MakeTravelTimeData.sh
# ${SRCDIR}/a06.BigProfile.sh
# ${SRCDIR}/a07.BigProfileComb.sh
# ${SRCDIR}/a08.BigProfileDistinctSum.sh
${SRCDIR}/a09.ZoomProfile.sh
# ${SRCDIR}/a50.MakePDF.sh

# ========= under construction =============
# ${SRCDIR}/a10.ZoomProfileComb.sh
# ${SRCDIR}/a11.ZoomProfileDistinctSum.sh
# ${SRCDIR}/a12.RadiationPat.sh
# ${SRCDIR}/a13.RadiationPatPiercings.sh
# ${SRCDIR}/a14.SNR.sh
# ${SRCDIR}/a15.EmpiricalSourceWavelets.sh
# ${SRCDIR}/a16.ZoomProfileDistinctSumWeight.sh
# ${SRCDIR}/a17.MapNetWK.sh
# ${SRCDIR}/a18.MapScSBounce.sh
# ${SRCDIR}/a19.MapSSBounce.sh
# ${SRCDIR}/a20.MapSdiffPath.sh
# ${SRCDIR}/a21.MapSKSInOutCMB.sh
# ${SRCDIR}/a22.MapPKIKPInOutCMB.sh

exit 0
