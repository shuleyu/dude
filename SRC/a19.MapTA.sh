...
do
	...
	# Ctrl+C action.
	trap "rm -f ${a19DIR}/${EQ}* ${PLOTFILE} ${OUTDIR}/*_${RunNumber}; exit 1" SIGINT
...
EOF
		...
		# plot topography.
		grdimage ${REG} ${PROJ} Topo.grd -CTopo.cpt -Sb -X1i -Y-7.5i -O -K >> ${PLOTFILE}
...
EOF
	...
	then
		...
		gmt grdsample ${SRCDIR}/etopo5.grd -GTopo.grd -I0.3/0.3
...
0 1 Event: ${MM}/${DD}/${YYYY} ${HH}:${MIN} USArray Transportable Array Stations
...
EOF
		...
		# plot topography.
		gmt grdimage ${REG} ${PROJ} Topo.grd -CTopo.cpt -nb -X1i -Y-7.5i -O -K >> ${PLOTFILE}
...
EOF
		...
		# info on some of these choices). again, thanks to Kevin Eagar for this
		# idea of using gmtmath w/ grdcontour for this application
		gmt grdcontour dist.grd ${PROJ} ${REG} -A5+f14p,Helvetica,red+ap -Gl-140/50/-60/0 \
            -C5 -S8 -W0.5,red -B0 -O >> ${PLOTFILE}
