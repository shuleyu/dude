...
do
	...
	# Ctrl+C action.
	trap "rm -f ${a16DIR}/${EQ}* ${a16DIR}/tmpfile*$$ ${OUTDIR}/*_${RunNumber}; exit 1" SIGINT
	...
	Num=0
	while read Phase COMP UseSNR DistMin DistMax AzMin AzMax F1 F2 NETWK TimeMin TimeMax TravelCurve PlotOrient
	do
		Num=$((Num+1))
		...
		then
			...
			echo "    ==> Making AlignedProfile plot of ${EQ}, Line ${Num}..."
		fi

		trap "rm -f ${a16DIR}/${EQ}* ${PLOTFILE} ${a16DIR}/tmpfile*$$ ${OUTDIR}/*_${RunNumber}; exit 1" SIGINT
...
EOF
		...
		paste ${EQ}_in_$$ tmpfile_gcarc_$$ > ${EQ}_AlignedIn_$$
		...
		AmpScale=`echo "${Amplitude_AP}/${PlotHeight}*(${DISTMAX}- ${DISTMIN})" | bc -l`
		rm -f tmpfile_$$


		# H. Make Plot file.

		${EXECDIR}/AlignedProfile.out 0 3 3 << EOF
${EQ}_AlignedIn_$$
${EQ}_PlotFile.txt
...
EOF
		...
		if [ $? -ne 0 ]
		then
			echo "    !=> AlignedProfile.out C++ code failed on ${EQ}, line ${Num} ..."
			rm -f ${a16DIR}/${EQ}* ${OUTFILE}
			exit 1
		fi
...
EOF
	...
	done < ${OUTDIR}/tmpfile_AP_${RunNumber}
