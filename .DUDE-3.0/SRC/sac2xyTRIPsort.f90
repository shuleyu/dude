...
IMPLICIT NONE
INTEGER, parameter            :: MAXnpts = 200000, MAXsac = 1000, PLOTpoints = 3001
INTEGER                       :: iSAC, nSAC, sacnpts, skip_points,ipts, iorder, npts_twin_around_reference 
...
REAL(kind=8)                  :: reduce,tempMAX, tempendtime, twin_around_reference, timeSTART 
REAL(kind=8)                  :: timebeforeP, timewin_noise, timewin_signal
REAL                          :: sacamp(MAXnpts), cuty(MAXnpts),gcarc(MAXsac),  sacbeg, sacdt,sacgcarc, gcarcB, reftime_flag
REAL, ALLOCATABLE             :: y(:,:), beg(:), dt(:), cutbeg(:), intp_y(:,:), intp_time(:,:), ordergcarc(:), MAXamp(:), reftime(:)
REAL, ALLOCATABLE             :: envarray(:,:)
REAL(kind=8), ALLOCATABLE     :: realtime_start(:), noiseP(:), signal(:), SNratio(:)
...
CHARACTER                     :: yntaper*1, ynnormalize*1, WAVE*1
   ...
   timebeforeP    = 180 !in sec
   timewin_noise  = 60
   timewin_signal = 20.0
   twin_around_reference = 20 !in sec
  ...
  open(14, file = "xy.seismograms.win")
   ...
   do iSAC = 1, nSAC
       ...
       !print *, cutbeg(iSAC) + pts_position_range_around_reference1*dt(iSAC)-reduce*ordergcarc(iSAC), &
       !         cutbeg(iSAC) + pts_position_range_around_reference2*dt(iSAC)-reduce*ordergcarc(iSAC), &
       !         MAXamp(iSAC)
   ...
   !mean_SNratio=MAXVAL(SNratio)
   open(12, file = "xy.seismograms")
   write(12,'(''> '',2(f12.6))')  ordergcarc(1), reftime(1)-reduce*ordergcarc(1) 
   do ipts=1,PLOTnpts(1)
       !write(12,*) intp_time(ipts,1), intp_y(ipts,1)/tempMAX*amp_multiply+ordergcarc(1), ipts
       !write(12,*) intp_time(ipts,1), intp_y(ipts,1)/MAXamp(1)*amp_multiply+ordergcarc(1), ipts
       write(12,*) intp_time(ipts,1), ordergcarc(1)-intp_y(ipts,1)/MAXamp(1)*SNratio(1)/mean_SNratio*amp_multiply, ipts
       !write(12,*) intp_time(ipts,1), ordergcarc(1)-intp_y(ipts,1)/MAXamp(1)*amp_multiply, ipts
   end do
   gcarcB = ordergcarc(1)
   do iSAC = 2, nSAC
       if (  (ordergcarc(iSAC) - gcarcB ) >= 0.05 .and. ABS(MAXamp(iSAC) - 0.0 ) >= 0.0001 ) then 
       !if (  (ordergcarc(iSAC) - gcarcB ) >= 0.0005 .and. ABS(MAXamp(iSAC) - 0.0 ) >= 0.0001 ) then 
           write(12,'(''> '',2(f12.6))')  ordergcarc(iSAC), reftime(iSAC)-reduce*ordergcarc(iSAC)
           do ipts=1,PLOTnpts(iSAC)
               !write(12,*) intp_time(ipts,iSAC), ordergcarc(iSAC)-intp_y(ipts,iSAC)/tempMAX*amp_multiply, ipts
               !write(12,*) intp_time(ipts,iSAC), ordergcarc(iSAC)-intp_y(ipts,iSAC)/MAXamp(iSAC)*amp_multiply, ipts
               write(12,*) intp_time(ipts,iSAC), ordergcarc(iSAC)-intp_y(ipts,iSAC)/MAXamp(iSAC)*SNratio(iSAC)/mean_SNratio &
                           *amp_multiply, ipts
           end do
   ...
   !do iSAC = 1, nSAC
   !    write(12,'(''> '',2(f8.3))')  ordergcarc(iSAC), dt(iSAC)
   !    do ipts=1,PLOTnpts(iSAC)
   !        !if ( intp_time(ipts,iSAC) .ge. beg(iSAC) .and. &
   !        !     intp_time(ipts,iSAC) .lt. beg(iSAC)+real(npts(iSAC))*dt(iSAC) ) then
   !            write(12,*) intp_time(ipts,iSAC), intp_y(ipts,iSAC)*amp_multiply+ordergcarc(iSAC) 
   !        !end if
   !    end do
   !end do
