PROGRAM sac2xyzoomstack_withweighting 
...
REAL                          :: weighted_value, weighted(MAXsac)
...
CHARACTER*10                  :: refphase, t_header
   ...
   do while ( status_read .eq. 0 )
       read(11,*,iostat = status_read ) sacname, weighted_value
   ...
   do iSAC = 1, nSAC
       ...
       do ipts = 1, PLOTnpts(iSAC)
          ! mul * -1.0 because on the plot, ytop=> shorter distance, ybottom=> large distance
          intp_y(ipts,iSAC)=-1.0*(cuty(1+(ipts-1)*skip_points)*weighted(iSAC))
