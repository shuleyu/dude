...
IMPLICIT NONE
INTEGER, parameter            :: MAXnpts = 200000, MAXsac = 1500, PLOTpoints = 1001
...
REAL, ALLOCATABLE             :: y(:,:), beg(:), dt(:), cutbeg(:), intp_y(:,:), intp_time(:,:), newbeg(:)
...
CHARACTER                     :: yntaper*1, ynnormalize*1
   ...
   ALLOCATE(ntrace_stack(ndstack), stack_amp_normalizeMAXtrace(MAXnpts,ndstack), newbeg(ndstack))
   ...
   do iSAC = 1, nSAC
       ...
       call SUB_CUT_SACascii( y(:,iSAC), beg(iSAC), dt(iSAC) , time_beg, time_win, &
       ...
       do ipts = 1, PLOTnpts(iSAC)
          ...
          intp_time(ipts,iSAC)=cutbeg(iSAC)+real((ipts-1)*skip_points)*dt(iSAC)
       end do
   end do
   ...
   do idstack = 1, ndstack
       ...
       do ipts = 1, PLOTnpts(1)
            if ( ntrace_stack(idstack) .ne. 0  .and.  intp_time(ipts,1) >= newbeg(idstack) ) then
